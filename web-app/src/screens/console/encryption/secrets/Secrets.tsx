// This file is part of MinIO KES
// Copyright (c) 2023 MinIO, Inc.
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <http://www.gnu.org/licenses/>.

import { Grid } from "@mui/material";
import {
  AddIcon,
  Button,
  DataTable,
  Grid as GridMDS,
  PageHeader,
  PageLayout,
  RefreshIcon,
  Tooltip,
} from "mds";
import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import { useAppDispatch } from "../../../../app/hooks";
import api from "../../../../common/api";
import { ErrorResponseHandler } from "../../../../common/api/types";
import { setErrorSnackMessage } from "../../../../systemSlice";
import SearchBox from "../../common/SearchBox";
import { ROUTES } from "../../valid-routes";

const DeleteModal = React.lazy(() => import("../DeleteModal"));

const ListSecrets = () => {
  const dispatch = useAppDispatch();
  const navigate = useNavigate();

  const [filter, setFilter] = useState<string>("");
  const [deleteOpen, setDeleteOpen] = useState<boolean>(false);
  const [selectedSecret, setSelectedSecret] = useState<string>("");
  const [loading, setLoading] = useState<boolean>(false);
  const [records, setRecords] = useState<[]>([]);

  // TODO: Use supported apis endpoint to check available apis
  const deleteSecret = true;
  const displaySecrets = true;

  useEffect(() => {
    fetchRecords();
  }, []);

  useEffect(() => {
    setLoading(true);
  }, [filter]);

  useEffect(() => {
    if (loading) {
      if (displaySecrets) {
        let pattern = filter.trim() === "" ? "*" : filter.trim();
        api
          .invoke("GET", `/api/v1/encryption/secrets?pattern=${pattern}`)
          .then((res) => {
            setLoading(false);
            setRecords(res.results);
          })
          .catch((err: ErrorResponseHandler) => {
            setLoading(false);
            dispatch(setErrorSnackMessage(err));
          });
      } else {
        setLoading(false);
      }
    }
  }, [loading, setLoading, setRecords, dispatch, displaySecrets, filter]);

  const fetchRecords = () => {
    setLoading(true);
  };

  const confirmDeleteSecret = (secret: string) => {
    setDeleteOpen(true);
    setSelectedSecret(secret);
  };

  const closeDeleteModalAndRefresh = (refresh: boolean) => {
    setDeleteOpen(false);

    if (refresh) {
      fetchRecords();
    }
  };

  const tableActions = [
    {
      type: "delete",
      onClick: confirmDeleteSecret,
      sendOnlyId: true,
      disableButtonFunction: () => !deleteSecret,
    },
  ];

  return (
    <React.Fragment>
      {deleteOpen && (
        <DeleteModal
          deleteOpen={deleteOpen}
          selectedItem={selectedSecret}
          withWarning={false}
          endpoint={"/api/v1/encryption/secrets/"}
          element={"Secret"}
          label={"Delete Secret"}
          closeDeleteModalAndRefresh={closeDeleteModalAndRefresh}
        />
      )}
      <PageHeader label="Secrets" />
      <PageLayout>
        <Grid container spacing={1}>
          <Grid
            item
            xs={12}
            display={"flex"}
            alignItems={"center"}
            justifyContent={"flex-end"}
            sx={{
              "& button": {
                marginLeft: "8px",
              },
            }}
          >
            <SearchBox
              onChange={setFilter}
              placeholder="Search Secrets with pattern"
              value={filter}
            />
            <Tooltip placement="bottom" tooltip={"Refresh"}>
              <Button
                id={"refresh-secrets"}
                variant="regular"
                icon={<RefreshIcon />}
                onClick={() => setLoading(true)}
              />
            </Tooltip>
            <Tooltip placement="bottom" tooltip={"Create Secret"}>
              <Button
                id={"create-secret"}
                label={"Create Secret"}
                variant={"callAction"}
                icon={<AddIcon />}
                onClick={() => navigate(ROUTES.ENCRYPTION_SECRETS_ADD)}
              />
            </Tooltip>
          </Grid>
          <Grid item xs={12}>
            <GridMDS item xs={12}>
              <DataTable
                itemActions={tableActions}
                columns={[
                  {
                    label: "Name",
                    elementKey: "name",
                  },
                  {
                    label: "Type",
                    elementKey: "secretType",
                  },
                  {
                    label: "Created by",
                    elementKey: "createdBy",
                  },
                  {
                    label: "Created at",
                    elementKey: "createdAt",
                  },
                  {
                    label: "Updated at",
                    elementKey: "updatedAt",
                  },
                ]}
                entityName="secrets"
                records={records}
                idField={"name"}
              />
            </GridMDS>
          </Grid>
        </Grid>
      </PageLayout>
    </React.Fragment>
  );
};

export default ListSecrets;
