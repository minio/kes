...
func complete(cmd string) bool {
	...
	completion := map[string][]string{
		cmd:             {"server", "edge", "enclave", "key", "policy", "identity", "log", "status", "metric", "update"},
		cmd + " server": {"--addr", "--host"},
		cmd + " edge":   {"--addr"},
		cmd + " log":    {"--audit", "--error", "--json", "--insecure"},
		cmd + " status": {"--short", "--api", "--json", "--insecure"},
		cmd + " metric": {"--rate", "--insecure"},
		cmd + " update": {"--downgrade", "--output", "--os", "--arch", "--minisign-key", "--insecure"},

		cmd + " cluster":      {"add", "info", "rm"},
		cmd + " cluster add":  {"--insecure"},
		cmd + " cluster info": {"--insecure", "--json"},
		cmd + " cluster rm":   {"--insecure"},

		cmd + " enclave":        {"create", "info", "ls", "rm"},
		cmd + " enclave create": {"--insecure"},
		cmd + " enclave info":   {"--insecure", "--json"},
		cmd + " enclave ls":     {"--insecure", "--json"},
		...
		cmd + " key info":    {"--enclave", "--insecure", "--json"},
		cmd + " key ls":      {"--enclave", "--insecure", "--json"},
		cmd + " key rm":      {"--enclave", "--insecure"},
		cmd + " key encrypt": {"--enclave", "--insecure"},
		cmd + " key decrypt": {"--enclave", "--insecure"},
		cmd + " key dek":     {"--enclave", "--insecure", "--json"},

		cmd + " policy":        {"create", "assign", "info", "ls", "rm", "show"},
		cmd + " policy create": {"--enclave", "--insecure"},
		cmd + " policy assign": {"--enclave", "--insecure"},
		cmd + " policy info":   {"--enclave", "--insecure", "--json"},
		cmd + " policy ls":     {"--enclave", "--insecure", "--json"},
		cmd + " policy rm":     {"--enclave", "--insecure"},
		cmd + " policy show":   {"--enclave", "--insecure", "--json"},

		cmd + " identity":        {"new", "of", "create", "info", "ls", "rm"},
		cmd + " identity new":    {"--key", "--cert", "--force", "--ip", "--dns", "--expiry", "--encrypt"},
		cmd + " identity of":     {"--json"},
		cmd + " identity create": {"--enclave", "--insecure", "--policy", "--admin", "--ttl"},
		cmd + " identity info":   {"--enclave", "--insecure", "--json"},
		cmd + " identity ls":     {"--enclave", "--insecure", "--json"},
