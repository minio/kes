...
func NewServerMux(config *ServerConfig) *http.ServeMux {
	...
	mux.Handle("/v1/log/audit", config.Metrics.Count(config.Metrics.Latency(audit(config.AuditLog.Log(), requireMethod(http.MethodGet, validatePath("/v1/log/audit", limitRequestBody(0, tlsProxy(config.Proxy, enforcePolicies(config, handleTraceAuditLog(config.AuditLog))))))))))
	mux.Handle("/v1/log/error", config.Metrics.Count(config.Metrics.Latency(audit(config.AuditLog.Log(), requireMethod(http.MethodGet, validatePath("/v1/log/error", limitRequestBody(0, tlsProxy(config.Proxy, enforcePolicies(config, handleTraceErrorLog(config.ErrorLog))))))))))
