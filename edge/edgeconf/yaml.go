...
func decode(y *yml) (*ConfigFile, error) {
	...
	cert, err := mtls.CertificateFromFile(y.TLS.Certificate.Value, y.TLS.PrivateKey.Value, y.TLS.Password.Value)
	if err != nil {
		return nil, fmt.Errorf("failed to load TLS certificate: %v", err)
	}

	var rootCAs *x509.CertPool
	if y.TLS.CAPath.Value != "" {
		rootCAs, err = mtls.CertPoolFromFile(y.TLS.CAPath.Value)
		if err != nil {
			return nil, fmt.Errorf("failed to load CA certificates: %v", err)
		}
	}

	clientAuth := tls.RequireAnyClientCert
	for _, api := range y.API.Paths {
	...
	if strings.TrimSpace(strings.ToLower(y.Log.Error.Value)) != "off" { // default is "on" behavior
		config.ErrorLog = slog.NewTextHandler(os.Stderr, nil)
	}
	if strings.TrimSpace(strings.ToLower(y.Log.Audit.Value)) == "on" { // default is "on" behavior
		config.AuditLog = slog.NewTextHandler(os.Stdout, nil)
	}

	if len(y.Policies) > 0 {
		config.Policies = make(map[string]*kes.Policy, len(y.Policies))
		config.Identities = make(map[kes.Identity]string)
		for name, policy := range y.Policies {
			for _, id := range policy.Identities {
				if _, ok := config.Identities[id.Value]; ok {
				}
				config.Identities[id.Value] = name
			}
		}
	}

	if len(y.API.Paths) > 0 {
		config.API = make(map[string]edge.APIConfig, len(y.API.Paths))
		for path, api := range y.API.Paths {
			config.API[path] = edge.APIConfig{
...
func decodeKeyStore(y *yml) (keystore, error) {
